...
public class MultiplaEscolhaVO extends ObjetivaVO {
	...
	public void addOpcao(OpcaoVO opcao) {
		if (opcao != null) {
			...
			if (super.getOpcoes().size() < 26) {
				...
				opcao.setConteudo(opcaoFormatada);
	...
	public void removeOpcao(int posicao) {
		if (posicao < super.getOpcoes().size()) {
			...
			if (super.getGabarito().equals(String.valueOf(super.getOpcoes().get(posicao).getConteudo().charAt(0)))) {
			 ...
			 * removida tenham as suas alternativas subtraidas por 1 for (int i = posicao; i
			 * < super.getOpcoes().size(); i++) { char alternativa =
			 * super.getOpcoes().get(i).getConteudo().charAt(0); alternativa--; String temp
			 * = super.getOpcoes().get(i).getConteudo().substring(3);
			 * 
	...
	private boolean testeGabaritoValido(String gabarito) {
		...
		while (teste == false && i < super.getOpcoes().size()) {
			...
			if (gabarito.equals(String.valueOf(super.getOpcoes().get(i).getConteudo().charAt(0)))) {
				teste = true;
			}
			i++;
		}
